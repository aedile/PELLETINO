...
def convert_program_rom(rom_dir, out_dir):
    ...
    rom_data = bytearray()
    extra_arrays = []  # additional (name, bytes) arrays to emit in the header
    ...
    if GAME_NAME == 'mspacman':
        ...
        if (rom_dir / 'boot1').exists():
            ...
            # Patch ROM variant (Pac-Man base + u5/u6/u7 aux-board ROMs)
            # Exact port of MAME's init_mspacman() / mspacman_install_patches()
            # (src/mame/pacman/pacman.cpp).
            #
            # Output layout (32KB "decoded" bank, what the Z80 sees with the
            # daughterboard latch SET):
            #   0x0000-0x3FFF -> Z80 0x0000-0x3FFF  patched Pac-Man code + decrypted u7
            #   0x4000-0x7FFF -> Z80 0x8000-0xBFFF  decrypted u5/u6 + Pac-Man ROM mirrors
            # plus a 16KB "plain" bank (latch CLEARED): the unpatched Pac-Man ROMs,
            # which the hardware also mirrors at 0x8000-0xBFFF.
            print("  Using patch ROM variant (pacman.6* + u5/u6/u7)...")
            print("  Applying MAME-style decryption...")

            def bitswap(val, nbits, *bits):
                """MAME bitswap<N>(val, bN-1 ... b0): new bit k <- old bit bits[N-1-k]."""
                assert len(bits) == nbits
                out = 0
                for k, src in enumerate(bits):
                    out |= ((val >> src) & 1) << (nbits - 1 - k)
                return out

            def decrypt_byte(b):
                return bitswap(b, 8, 0, 4, 5, 7, 6, 3, 2, 1)
            ...
            for rom_file, addr in base_roms:
                ...
                data = read_rom_file(path)
                if len(data) != 0x1000:
                    print(f"  ERROR: {rom_file} is {len(data)} bytes, expected 4096")
                    return False
                plain[addr:addr + 0x1000] = data
                print(f"  {rom_file}: {len(data)} bytes @ 0x{addr:04X}")

            aux = {}
            for name, size in (('u5', 0x800), ('u6', 0x1000), ('u7', 0x1000)):
                path = rom_dir / name
                if not path.exists():
                    print(f"  ERROR: Missing aux ROM {name}")
                    return False
                aux[name] = read_rom_file(path)
                if len(aux[name]) != size:
                    print(f"  ERROR: {name} is {len(aux[name])} bytes, expected {size}")
                    return False
                print(f"  {name}: {len(aux[name])} bytes")

            # MAME source address space: u5@0x8000, u6@0x9000, u7@0xB000
            src = bytearray(0x10000)
            src[0x0000:0x4000] = plain
            src[0x8000:0x8800] = aux['u5']
            src[0x9000:0xA000] = aux['u6']
            src[0xB000:0xC000] = aux['u7']

            drom = bytearray(0x10000)  # MAME DROM (decoded bank), full 64KB
            for i in range(0x1000):
                drom[0x0000 + i] = src[0x0000 + i]  # pacman.6e
                drom[0x1000 + i] = src[0x1000 + i]  # pacman.6f
                drom[0x2000 + i] = src[0x2000 + i]  # pacman.6h
                # decrypt u7 -> 0x3000
                a = bitswap(i, 12, 11, 3, 7, 9, 10, 8, 6, 5, 4, 2, 1, 0)
                drom[0x3000 + i] = decrypt_byte(src[0xB000 + a])
            for i in range(0x800):
                a5 = bitswap(i, 11, 8, 7, 5, 9, 10, 6, 3, 4, 2, 1, 0)
                a6 = bitswap(i, 11, 3, 7, 9, 10, 8, 6, 5, 4, 2, 1, 0)
                drom[0x8000 + i] = decrypt_byte(src[0x8000 + a5])  # decrypt u5
                drom[0x8800 + i] = decrypt_byte(src[0x9800 + a6])  # decrypt half of u6
                drom[0x9000 + i] = decrypt_byte(src[0x9000 + a6])  # decrypt half of u6
                drom[0x9800 + i] = src[0x1800 + i]                 # mirror of pacman.6f high
            for i in range(0x1000):
                drom[0xA000 + i] = src[0x2000 + i]  # mirror of pacman.6h
                drom[0xB000 + i] = src[0x3000 + i]  # mirror of pacman.6j
            print("  Decrypted u5/u6/u7")

            # mspacman_install_patches(): copy forty 8-byte patches into Pac-Man code
            patches = [
            ...
            for dst, srcaddr in patches:
                drom[dst:dst + 8] = drom[srcaddr:srcaddr + 8]
            print(f"  Applied {len(patches)} 8-byte patches")

            # Pack into the emulator image: [0x0000-0x3FFF] + [0x8000-0xBFFF]
            rom_data = bytearray(drom[0x0000:0x4000] + drom[0x8000:0xC000])
            extra_arrays.append(('mspacman_rom_plain', bytes(plain)))
            print(f"  Decoded bank: {len(rom_data)} bytes, plain bank: {len(plain)} bytes")
        else:
    ...
    for extra_name, extra_data in extra_arrays:
        output += ['', to_c_array(extra_data, extra_name)]
    output += [
        '',
        f'#endif // {guard_name}',
    ]
